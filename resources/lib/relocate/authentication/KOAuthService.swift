...
final class KOAuthService: AuthService {
  ...
  private let keyValueService: KVSService,
  ...
  func update(_ user: User) async throws -> User {
    try await executeIfAuthenticated { _ in
      try await mapCloudError {
        let user = try await remoteDBService.publish(user)
        status = .authenticated(user)
        return user
      }
    }
  ...
  func update(_ credential: Credential) async throws -> Credential {
    try await executeIfAuthenticated { _ in
      try await mapCloudError {
        save(credential: credential)
        return try await remoteDBService.publish(credential)
      }
    }
  }
  ...
  func deleteAccount() async throws {
    try await executeIfAuthenticated { user in
      try await mapCloudError {
        logout()
        try await remoteDBService.unpublish(user)
        try await remoteDBService.unpublish(Credential(userID: user.id, pin: ""))
      }
    }
  }
}
...
private extension KOAuthService {
  ...
  func createUser(for credential: Credential) async throws -> User {
    ...
    try await remoteDBService.publish(user)
    try await remoteDBService.publish(credential)

    return user
  }

  func check(_ credential: Credential) async throws {
    guard let actualCredential: Credential = try await remoteDBService.fetch(with: credential.id) else {
  ...
  func getUser(with credential: Credential) async throws -> User? {
    return try await remoteDBService.fetch(with: credential.userID)
  }
  ...
  func isTaken(id: String) async throws -> Bool {
    try await remoteDBService.exists(with: id, User.self)
  }
...
private extension KOAuthService {
  ...
  func save(credential: Credential) {
    printError {
      keyValueService.store(credential.id, for: Self.userIDKey)
      if let pin = credential.pin {
        keyValueService.store(pin, for: Self.userPinKey)
      } else {
        keyValueService.store(true, for: Self.pinlessKey)
      }
    }
  }
  ...
  func readCredential() -> Credential? {
    printError {
      if let id: String = keyValueService.load(for: Self.userIDKey) {
        if let pinless: Bool = keyValueService.load(for: Self.pinlessKey), pinless {
          return Credential(id: id, pin: nil)
        } else if let pin: String = keyValueService.load(for: Self.userPinKey) {
  ...
  func deleteCredential() {
    printError {
      keyValueService.delete(for: Self.userIDKey)
      keyValueService.delete(for: Self.userPinKey)
    }
  }
}

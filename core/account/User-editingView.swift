...
extension User {
  ...
  struct EditingView: View {
    var body: some View {
      ...
      .onChange(of: mainState.authenticationState.user) { newUser in
...
private extension User.EditingView {
  ...
  @MainActor func saveColorID(_ colorID: ColorID) async {
    await printError {
      ...
      try await mainState.publicDBService.modify(User.self, with: user.id) { mutable in mutable.colorID = colorID }
    }
  }
  ...
  @MainActor func saveName() async {
    await printError {
      ...
      try await mainState.publicDBService.modify(User.self, with: user.id) { mutable in mutable.name = name }
